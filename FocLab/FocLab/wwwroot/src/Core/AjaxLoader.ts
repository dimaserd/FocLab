class AjaxLoader {
    ...
    static InitAjaxLoads = function () {
        const elems = document.getElementsByClassName("ajax-load-html");
        for (let i = 0; i < elems.length; i++) {
            AjaxLoader.LoadInnerHtmlToElement(elems[i], () => 2);
        }
    }


    static LoadInnerHtmlToElement = function (element: Element, onSuccessFunc: Function) : void {
