...
using NewFocLab.Logic.Extensions;
using NewFocLab.Model.Entities;

namespace NewFocLab.Logic.Services.ChemistryTaskExperiments
{
    /// <summary>
    /// Статический класс предоставляющий методы для работы с экспериментами
    /// </summary>
    public class ChemistryTaskExperimentsWorker : FocLabService
    {
        /// <summary>
        /// Конструктор
        /// </summary>
        ...
        public async Task<BaseApiResponse> PerformExperimentAsync(PerformExperimentModel model)
        {
            ...
            if (model.Performed)
            {
                ...
                await PublishMessageAsync(new ExperimentPerformedEvent
                {
                    Id = experiment.Id
                });
